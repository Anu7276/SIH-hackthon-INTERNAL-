...
async function handleRegister(e) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById('target-otp-email').innerText = email;
            switchTab('otp');
        } else {
...
function renderDashboard(user, token) {
    ...
    if (navLink) {
        navLink.innerHTML = `<span style="color:#2e6b3e;font-weight:700;"><i class="fa-solid fa-circle-user" style="margin-right:4px;"></i> ${user.username}</span>`;
    }
